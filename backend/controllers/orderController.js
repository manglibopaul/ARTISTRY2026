    ...
    for (const groupKey of sellerGroupKeys) {
      ...
      try {
        ...
        if (dialect === 'sqlite') {
          ...
          hasGcashColumn = Array.isArray(res) && res.length > 0;
        }
      } catch (e) {
...
export const getSellerDashboardStats = async (req, res) => {
  try {
    const sellerId = Number(req.seller.id);
    ...
    const lowStock = products.filter(p => Number(p.stock) > 0 && Number(p.stock) <= lowStockThreshold).length;
    const outOfStock = products.filter(p => !p.stock || Number(p.stock) <= 0).length;

    // Fetch recent orders (limited) and compute seller-specific metrics
    const limit = 1000;
    let orders = [];
    try {
      orders = await Order.findAll({ order: [['createdAt', 'DESC']], limit });
    ...
    const sellerOrders = [];
    const productSales = {}; // productId -> { qty, revenue, name }
    let totalOrders = 0;
    let totalRevenue = 0;
    ...
    for (const order of orders) {
      ...
      const sellerItems = items.filter(it => Number(it.sellerId) === sellerId);
      if (sellerItems.length === 0) continue;

      totalOrders++;

      // compute seller portion of this order
      let orderSellerTotal = 0;
      const mappedItems = sellerItems.map(it => {
        const quantity = Number(it.quantity || 1);
        const price = Number(it.price || 0);
        const lineTotal = quantity * price;
        orderSellerTotal += lineTotal;

        // aggregate per product
        const prodId = String(it.productId || it.id || it._id || 'unknown');
        productSales[prodId] = productSales[prodId] || { qty: 0, revenue: 0, name: it.name || '' };
        productSales[prodId].qty += quantity;
        productSales[prodId].revenue += lineTotal;
      ...
      sellerOrders.push({ id: order.id, orderStatus: order.orderStatus, paymentMethod: order.paymentMethod, createdAt: order.createdAt, sellerItems: mappedItems });
    }

    // Best selling products (top 6)
    const bestSelling = Object.keys(productSales)
      .map(pid => ({ productId: pid, ...productSales[pid] }))
    ...
    console.error('getSellerDashboardStats error', error && error.message ? error.message : error);
    res.status(500).json({ message: error.message || 'Failed to compute seller stats' });
  }
};

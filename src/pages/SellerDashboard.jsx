      ...
      <div className='max-w-7xl mx-auto px-3 sm:px-4 py-4 sm:py-8'>
            ...
            <form onSubmit={handleSubmit} className='grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6'>
              ...
              <div className='col-span-1 md:col-span-2 space-y-4'>
                <div>
                  ...
                  />
                  {formData.model && (
                    <p className='mt-2 text-sm text-green-600'>Γ£ô {formData.model.name}</p>
                  )}
                ...
                <div>
                  ...
                  />
                  {formData.iosModel && (
                    <p className='mt-2 text-sm text-green-600'>Γ£ô {formData.iosModel.name}</p>
                  )}
              ...
              >
                {isSubmitting ? (
                  <>
                    <span className='inline-block mr-2'>ΓÅ│</span>
                      ...
                      {products.map((product) => (
                        <tr key={product.id} className='border-b border-gray-200 hover:bg-gray-50'>
                          ...
                          <td className='px-6 py-4 text-sm'>
                            {(() => {
                              const stock = product.stock ?? 0
                              if (product.hidden) return <span className='inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600'>Γ¥î Hidden</span>
                              if (stock <= 0) return <span className='inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-700'>ΓÅ╕ Out of Stock</span>
                          ...
                          return (
                            <div key={idx} className='flex items-center gap-2'>
                              ...
                              <div className='text-xs text-gray-700'>
                                ...
                                <p className='text-gray-500'>x{it.quantity ?? 1} ┬╖ Γé▒{it.price ?? ''}</p>
                              </div>
                            </div>
                          )
                        })}
                              ...
                              return (
                                <div key={idx} className='flex items-center gap-3 mb-2'>
                                  ...
                                  <div>
                                    ...
                                    <div className='text-xs text-gray-500'>x{it.quantity ?? it.qty ?? it.q ?? 1} ┬╖ Γé▒{it.price ?? it.unitPrice ?? ''}</div>
                                  </div>
                                </div>
                              )
                            })}
                          </td>
                          <td className='px-6 py-4 text-sm text-gray-700'>Γé▒{order.total}</td>
                          <td className='px-6 py-4 text-sm text-gray-700'>
                            <select
                              value={order.orderStatus}
                              onChange={e => {
        ...
        {selectedTab === 'chat' && (
          <div className='bg-white rounded-lg shadow-lg p-6'>
            ...
            <div>
              <div className='mb-4 text-sm text-gray-600'>Messages with customers ΓÇö view conversations and reply below.</div>
              <div>
        ...
        {selectedTab === 'shipping' && (
          <div className='bg-white rounded-lg shadow-lg p-6'>
            ...
            <div className='space-y-6'>
              ...
              <div>
                <label className='block text-sm font-medium text-gray-700 mb-1'>Free Shipping Minimum Order (Γé▒)</label>
                <input
                  type='number'
                  min='0'
                  ...
                  {(shippingSettings.shippingRates || []).map((rate, idx) => (
                    <div key={idx} className='border rounded-lg p-4 bg-gray-50'>
                      <div className='grid grid-cols-1 sm:grid-cols-3 gap-3'>
                        ...
                        <div>
                          <label className='block text-xs text-gray-500 mb-1'>Price (Γé▒)</label>
                          <input
                            type='number'
                            min='0'
                            value={rate.price || 0}
                      ...
                      <div key={ret.id} className='border rounded-lg p-4'>
                        <div className='flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-3'>
                          <div>
                            ...
                            <div className='text-sm text-gray-600'>{ret.buyerName || 'Customer'} ΓÇö {ret.buyerEmail || ''}</div>
                            <div className='text-xs text-gray-400 mt-1'>{new Date(ret.createdAt).toLocaleString()}</div>
                          </div>
                          <div className='text-right'>
                            <div className='text-sm font-medium'>Refund: Γé▒{(ret.refundAmount || 0).toFixed(2)}</div>
                          </div>
                        </div>
                          ...
                          <div className='space-y-1'>
                            {(Array.isArray(ret.items) ? ret.items : []).map((it, idx) => (
                              <div key={idx} className='text-sm text-gray-600 flex justify-between'>
                                ...
                                <span>Γé▒{((it.price || 0) * (it.quantity || 1)).toFixed(2)}</span>
                              </div>
                            ))}
                          </div>
                        </div>
          ...
          <div className='fixed inset-0 z-50 flex items-end sm:items-center justify-center'>
            ...
            <div className='bg-white rounded-t-2xl sm:rounded-lg shadow-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto p-4 sm:p-6 relative z-10'>
              <div className='flex items-start justify-between mb-4'>
                ...
                <div>
                  <button onClick={() => setViewOrder(null)} className='text-sm text-gray-600'>Close Γ£û</button>
                </div>
              </div>
              ...
              <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                ...
                <div>
                  ...
                  <div className='text-sm'>Subtotal: Γé▒{viewOrder.subtotal}</div>
                  {viewOrder.paymentMethod !== 'pickup' && (
                    <div className='text-sm'>Shipping: Γé▒{viewOrder.shippingFee ?? 0}</div>
                  )}
                  <div className='text-sm'>Total: Γé▒{viewOrder.total}</div>
                ...
                <div className='space-y-2'>
                  {Array.isArray(viewOrder.sellerItems) && viewOrder.sellerItems.map((it, idx) => (
                    <div key={idx} className='p-3 border rounded flex items-center justify-between'>
                      ...
                      <div className='text-sm'>Γé▒{it.price}</div>
                    </div>
                  ))}
                </div>
              </div>

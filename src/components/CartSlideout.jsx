                  ...
                  <div key={index} className='pb-6 border-b'>
                    <div className='grid grid-cols-[80px_1fr_auto] gap-4'>
                      <div className='flex gap-4 col-span-2'>
                        <img src={imgSrc} alt={productData.name} className='w-20 h-20 object-cover rounded' />
                        <div className='flex flex-col justify-between'>
                          <div>
                            ...
                            {color && <p className='text-xs text-gray-500 mt-1'>Color: {color}</p>}
                            {size && <p className='text-xs text-gray-500 mt-1'>Size: {size}</p>}
                          </div>
                          <div className='flex items-center gap-2 mt-2'>
                            <button onClick={() => updateQuantity(item._id, item.quantity - 1)} className='w-8 h-8 flex items-center justify-center border border-gray-300 rounded hover:bg-gray-100 transition-colors'>−</button>
                            <span className='w-8 text-center font-medium'>{item.quantity}</span>
                            <button onClick={() => updateQuantity(item._1d, item.quantity + 1)} className='w-8 h-8 flex items-center justify-center border border-gray-300 rounded hover:bg-gray-100 transition-colors'>+</button>
                            <button onClick={() => updateQuantity(item._id, 0)} className='ml-2 text-gray-400 hover:text-red-500 transition-colors'>
                      ...
                      <div className='text-right font-semibold'>{currency}{itemTotal.toFixed(2)}</div>
                    </div>
                    {productData.sellerId && sellers[productData.sellerId] && (
                      <button onClick={(e) => { e.preventDefault(); onClose(); navigate(getArtisanPath(sellers[productData.sellerId])); }} className='mt-3 w-full text-left p-3 bg-gray-50 rounded border border-gray-200 hover:border-black hover:bg-white transition'>
